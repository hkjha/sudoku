     ...
     for(i=0;i<9;i++)
                      for(j=0;j<9;j++){
                                       ...
                                       for(k=1;k<=9;k++){
                                             ...
                                             for(l=0;l<9;l++) if(A[i][l]==k){ placement=0; break;} //if any value matches in the row, can't place 
                                             if (placement) for(l=0;l<9;l++) if(A[l][j]==k) { placement=0; break;}//if any value matches in the column, can't place
                                             if(placement){
                                             m=(i/3)*3; n=(j/3)*3;
                                             ...
                                             A[i][j]=0; //if the solution is not possible, put (i,j) back as empty
                                             if(x==1) {total_count++; if(total_count%100000==0) printf("\n%d",total_count); } //if it is solved, then no need to further check for next values of k
                                        } // end of k loop
